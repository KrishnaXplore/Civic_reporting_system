...
const NAV = [
  ...
  { key: 'complaints', icon: '≡', label: 'Recent Complaints' },
];
...
const StateAdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    api.get('/api/v1/admin/stats/location', { params: { state } })
      .then(({ data: res }) => setData(res.data))
      .catch(() => {})
      .finally(() => setLoading(false));
  }, [state]);

  const pieData = data?.byCategory?.map((c) => ({ name: c.name, value: c.count })) || [];

  return (
    <div style={{ minHeight: '100vh', background: '#F8FAFC', display: 'flex', fontFamily: "'DM Sans', sans-serif" }}>
      <link href="https://fonts.googleapis.com/css2?family=DM+Sans:opsz,wght@9..40,400;9..40,500;9..40,600;9..40,700&display=swap" rel="stylesheet" />

      <Sidebar navItems={NAV} activeTab={tab} onTabChange={setTab} accentColor="#8B5CF6" roleLabel="State Admin" />
      ...
      <main style={{ marginLeft: 240, flex: 1, padding: '36px 40px' }}>
        <div style={{ marginBottom: 28 }}>
          ...
          <p style={{ fontSize: 13, color: '#94A3B8', marginTop: 4 }}>State-wide complaint oversight</p>
        </div>
            ...
            {tab === 'overview' && (
              <>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 16, marginBottom: 28 }}>
                  <StatCard label="Total Complaints" value={data.total} accent="#8B5CF6" />
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                  <div style={{ background: '#fff', borderRadius: 16, padding: '24px 28px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
                    ...
                    {pieData.length > 0 ? (
                      ...
                      <div style={{ height: 220, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94A3B8', fontSize: 13 }}>No data</div>
                    )}
                  </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ borderBottom: '1px solid #F1F5F9' }}>
                      {['Title', 'Category', 'City', 'Status', 'Date'].map((h) => (
                  ...
                  <tbody>
                    {(data.recentComplaints || []).map((c) => (
                      <tr key={c._id} style={{ borderBottom: '1px solid #F8FAFC' }}
                        ...
                        <td style={{ padding: '13px 20px', fontSize: 12, color: '#94A3B8' }}>{c.locationDetails?.city || '—'}</td>
